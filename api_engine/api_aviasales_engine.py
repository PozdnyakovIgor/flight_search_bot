...
def build_url_top_cheapest_tickets(origin: str, departure_at: str = None,
                                   return_at: str = None, limit: int = 5) -> str:
    url = (f'{AVIASALES_BASE_URL}v3/prices_for_dates?origin={origin}'
...
def send_request_top_cheapest_tickets(origin: str, departure_at: str = None,
                                      return_at: str = None, limit: int = 5) -> json:
    response = requests.get(
    ...
    if len(response['data']):
        ...
        for ticket in response:
            ...
            if 'return_at' in ticket:
                tickets += (f'Дата и время обратного рейса: '
                            f'{format_date(ticket["return_at"])}\n')

            tickets += (f'Цена (руб): {ticket["price"]}\n'
